...
prime(Number) :- integer(Number), !, prime(Number, 2).
prime(Number, Helper) :- X is sqrt(Number), Helper >= X, !.
prime(Number, Helper) :- GCD is gcd(Number, Helper), GCD = 1, Helper1 is Helper + 1, prime(Number, Helper1).